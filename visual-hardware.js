(() => {
  ...
  function svg(kind, small=false){
    ...
    if(kind==='m2') return `<svg ${common}><rect x="8" y="28" width="${w-22}" height="${h-56}" rx="3" fill="#1c5f4c" stroke="#62ad8d"/><rect x="18" y="33" width="34" height="${h-66}" fill="#151f1c"/><rect x="58" y="33" width="34" height="${h-66}" fill="#151f1c"/><circle cx="${w-20}" cy="${h/2}" r="5" fill="none" stroke="#b5c3bc"/><g fill="#d0b65f">${Array.from({length:10},(_,i)=>`<rect x="${8+i*5}" y="${h-31}" width="3" height="6"/>`).join('')}</g></svg>`;
